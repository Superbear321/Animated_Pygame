...
# Fonts
font_xs = load_font(current_path  + '/assets/fonts/Comicsans.ttf', 16)
font_sm = load_font(current_path  + '/assets/fonts/Comicsans.ttf', 32)
font_md = load_font(current_path  + '/assets/fonts/Comicsans.ttf', 48)
font_lg = load_font(current_path  + '/assets/fonts/Comicsans.ttf', 64)
font_xl = load_font(current_path  + '/assets/fonts/Cheri.ttf', 80)

# Sounds
jump_snd = load_sound(current_path + '/assets/sounds/jump.ogg')
gem_snd = load_sound(current_path + '/assets/sounds/gem.ogg')
...
item_images = { "Gem": load_image('/assets/images/items/platformPack_item008.png'),
                "Key_y": load_image('/assets/images/items/platformPack_item014.png')}

door_images = { "Door_close" : load_image('/assets/images/tiles/platformPack_tile059.png')}

bullet_image = load_image('/assets/images/items/bullet.png')
# Levels
levels = [current_path  + '/assets/levels/level_1.json',
          current_path  + '/assets/levels/level_2.json',
          current_path  + '/assets/levels/level_3.json']
...
class Hero(pygame.sprite.Sprite):
    ...
    def move_and_check_tiles(self, level):
        coliables = pygame.sprite.Group()
        coliables.add(level.main_tiles)
        coliables.add(level.doors)
        # level.main_tiles + level.doors
        self.rect.x += self.vx
        if self.has_key:
            hit_list = pygame.sprite.spritecollide(self, level.main_tiles, False)
        else:
            hit_list = pygame.sprite.spritecollide(self, coliables, False)
    ...
    def process_doors(self, level):
        if self.has_key == True:
            hit_list = pygame.sprite.spritecollide(self, level.doors, True)
            for hit in hit_list:
                hit.kill()
                hit.apply(self)
...
class BasicEnemy(pygame.sprite.Sprite):
    ...
    def update(self, level):
        ...
        self.move_and_check_tiles(level)
        self.check_world_edges(level)
        self.check_bullets(level.bullets)
...
class Level():
    ...
    def load_music(self):
        pygame.mixer.music.load(current_path  + '/' + self.map_data['music'])
    ...
    def load_doors(self):
        ...
        for element in self.map_data['doors']:
            x = element[0] * self.scale
            y = element[1] * self.scale
            kind = element[2]

            if kind == "Door_close":
                t = Door(x, y, door_images[kind])           
...
class Game():
    ...
    def load_level(self):
        ...
        self.active_sprites.add(self.hero, self.level.items, self.level.doors, self.level.enemies)

        self.hero.gem_counter = 0
    ...
    def process_input(self):     
        ...
        if self.stage == Game.PLAYING:
            if pygame.mouse.get_pressed()[0]:
                self.hero.shoot(self.level, self)
